...
void adc_init(){
	...
	ADCSRA_reg	|= ( 1<<ADEN_bit | 1<<ADPS1_bit | 1<<ADPS2_bit ); 
	ADCSRA_reg	&= ~( 1<<ADPS0_bit | 1<<ADSC_bit | 1<<ADATE_bit | 1<<ADIE_bit | 1<<ADIF_bit ) ;
...
unsigned char read_adc_converted_data(void){
	...
	if(ADMUX_reg & 1<< ADLAR_bit == 0x00)
		{ adc_8bit_data = ADCL_reg;}	
