...
{
	char toOct[4] ;
	sprintf(toOct,"%03o",num);

	lcd_string(row_num,col_num,toOct);
}
...
{	char deci[4];
	sprintf(deci,"%03d",num);
	lcd_string(row_num,col_num,deci);
}
...
{
	char toHex[3];
	sprintf(toHex,"%02X",num);
	lcd_string(row_num,col_num,toHex);
}
